...
- Treat `DESIGN.md`'s `Header Section / Atmosphere-to-Page Background` as the source
  of truth for browser/mobile safe-area color handling, edge-to-edge viewport
  meta, iOS top-edge tint sampling, muted theme colors, manifest color, visible
  page background, header identity colors, fixed-length top atmosphere fade,
  light/dark page-atmosphere assets, and OG/social image generation.
